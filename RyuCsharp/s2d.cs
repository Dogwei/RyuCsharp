...
partial class Ryu
{
    ...
    {
        return Unsafe.As<uint64_t, double>(ref bits);
    }

    public static Status s2d_n(ref char buffer, int len, out double result)
    {
        result = 0;
        ...
        for (; i < len; i++)
        {
            char c = Unsafe.Add(ref buffer, i);
            if (c == '.')
            {
                if (dotIndex != len)
                {
        ...
        if (i < len && ((Unsafe.Add(ref buffer, i) == 'e') || (Unsafe.Add(ref buffer, i) == 'E')))
        {
            eIndex = i;
            i++;
            if (i < len && ((Unsafe.Add(ref buffer, i) == '-') || (Unsafe.Add(ref buffer, i) == '+')))
            {
                signedE = Unsafe.Add(ref buffer, i) == '-';
                i++;
            }
            for (; i < len; i++)
            {
        ...
        if (e10 >= 0)
        {
            ...
            m2 = mulShift64(m10, ref DOUBLE_POW5_SPLIT[(uint)e10, 0], j);
        ...
        else
        {
            ...
            m2 = mulShift64(m10, ref DOUBLE_POW5_INV_SPLIT[(uint)(-e10), 0], j);
    ...
    static Status s2d(ref char buffer, out double result)
    {
        return s2d_n(ref buffer, strlen(ref buffer), out result);
    }
}