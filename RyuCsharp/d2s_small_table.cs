...
partial class Ryu
{
    static readonly uint64_t[,] DOUBLE_POW5_INV_SPLIT2 = new uint64_t[13, 2] {
    ...
    static readonly uint64_t[,] DOUBLE_POW5_SPLIT2 = new uint64_t[13, 2] {
    ...
    static void double_computePow5(uint32_t i, ref uint64_t result)
    {
        uint32_t @base = i / POW5_TABLE_SIZE;
        uint32_t base2 = @base * POW5_TABLE_SIZE;
        uint32_t offset = i - base2;
        ref uint64_t mul = ref DOUBLE_POW5_SPLIT2[@base, 0];
        if (offset == 0)
        {
            Unsafe.Add(ref result,0 ) = Unsafe.Add(ref mul, 0);
            Unsafe.Add(ref result, 1) = Unsafe.Add(ref mul, 1);
            return;
        }
        uint64_t m = DOUBLE_POW5_TABLE[offset];
        uint64_t high1;
        uint64_t low1 = umul128(m, Unsafe.Add(ref mul, 1), out high1);
        uint64_t high0;
        uint64_t low0 = umul128(m, Unsafe.Add(ref mul, 0), out high0);
        ...
        Unsafe.Add(ref result, 0) = shiftright128(low0, sum, delta) + ((POW5_OFFSETS[i / 16] >> (int)((i % 16) << 1)) & 3);
        Unsafe.Add(ref result, 1) = shiftright128(sum, high1, delta);
    }

    // Computes 5^-i in the form required by Ryu, and stores it in the given pointer.
    static void double_computeInvPow5(uint32_t i, ref uint64_t result)
    {
        uint32_t @base = (i + POW5_TABLE_SIZE - 1) / POW5_TABLE_SIZE;
        uint32_t base2 = @base * POW5_TABLE_SIZE;
        uint32_t offset = base2 - i;
        ref uint64_t mul = ref DOUBLE_POW5_INV_SPLIT2[@base, 0]; // 1/5^base2
        if (offset == 0)
        {
            Unsafe.Add(ref result, 0) = Unsafe.Add(ref mul, 0);
            Unsafe.Add(ref result, 1) = Unsafe.Add(ref mul, 1);
        ...
        Unsafe.Add(ref result, 0) = shiftright128(low0, sum, delta) + 1 + ((POW5_INV_OFFSETS[i / 16] >> (int)((i % 16) << 1)) & 3);
        Unsafe.Add(ref result, 1) = shiftright128(sum, high1, delta);
    }
}