...
namespace RyuCsharp
{
    ...
    {
        static readonly native_two_dim_array<uint64_t> DOUBLE_POW5_INV_SPLIT2 = new uint64_t[13, 2] {
        ...
        static readonly native_one_dim_array<uint32_t> POW5_INV_OFFSETS = new uint32_t[19]{
        ...
        static readonly native_two_dim_array<uint64_t> DOUBLE_POW5_SPLIT2 = new uint64_t[13, 2] {
        ...
        static readonly native_one_dim_array<uint32_t> POW5_OFFSETS = new uint32_t[21] {
        ...
        static readonly native_one_dim_array<uint64_t> DOUBLE_POW5_TABLE = new uint64_t[POW5_TABLE_SIZE] {
        ...
        static void double_computePow5(uint32_t i, uint64_t* result)
        {
            uint32_t @base = i / POW5_TABLE_SIZE;
            uint32_t base2 = @base * POW5_TABLE_SIZE;
            uint32_t offset = i - base2;
            uint64_t* mul = DOUBLE_POW5_SPLIT2[@base];
            if (offset == 0)
            {
                result[0] = mul[0];
                result[1] = mul[1];
            ...
            result[0] = shiftright128(low0, sum, delta) + ((POW5_OFFSETS[i / 16] >> (int)((i % 16) << 1)) & 3);
            result[1] = shiftright128(sum, high1, delta);
        }

        // Computes 5^-i in the form required by Ryu, and stores it in the given pointer.
        static void double_computeInvPow5(uint32_t i, uint64_t* result)
        {
            uint32_t @base = (i + POW5_TABLE_SIZE - 1) / POW5_TABLE_SIZE;
            uint32_t base2 = @base * POW5_TABLE_SIZE;
            uint32_t offset = base2 - i;
            uint64_t* mul = DOUBLE_POW5_INV_SPLIT2[@base]; // 1/5^base2
            if (offset == 0)
            {
                result[0] = mul[0];
                result[1] = mul[1];
            ...
            result[0] = shiftright128(low0, sum, delta) + 1 + ((POW5_INV_OFFSETS[i / 16] >> (int)((i % 16) << 1)) & 3);
            result[1] = shiftright128(sum, high1, delta);
        }
    }
}