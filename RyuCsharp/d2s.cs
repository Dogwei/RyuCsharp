...
partial class Ryu
{
    ...
    static floating_decimal_64 d2d(uint64_t ieeeMantissa, uint32_t ieeeExponent)
    {
        ...
        if (e2 >= 0)
        {
            ...
            vr = mulShiftAll64(m2, ref DOUBLE_POW5_INV_SPLIT[q, 0], i, out vp, out vm, mmShift);
        ...
        else
        {
            ...
            vr = mulShiftAll64(m2, ref DOUBLE_POW5_SPLIT[(uint)i, 0], j, out vp, out vm, mmShift);
            if (q <= 1)
            {
    ...
    static int to_chars(floating_decimal_64 v, bool sign, ref char result)
    {
        ...
        if ((output >> 32) != 0)
        {
            ...
            uint32_t d0 = (d % 100) << 1;
            uint32_t d1 = (d / 100) << 1;
            memcpy(ref Unsafe.AddByteOffset(ref result, (nint)(index + olength - i - 1) * sizeof(char)), ref DIGIT_TABLE[c0], 2);
            memcpy(ref Unsafe.AddByteOffset(ref result, (nint)(index + olength - i - 3) * sizeof(char)), ref DIGIT_TABLE[c1], 2);
            memcpy(ref Unsafe.AddByteOffset(ref result, (nint)(index + olength - i - 5) * sizeof(char)), ref DIGIT_TABLE[d0], 2);
            memcpy(ref Unsafe.AddByteOffset(ref result, (nint)(index + olength - i - 7) * sizeof(char)), ref DIGIT_TABLE[d1], 2);
            i += 8;
        }
        ...
        if (output2 >= 100)
        {
            ...
            output2 /= 100;
            memcpy(ref Unsafe.AddByteOffset(ref result, (nint)(index + olength - i - 1) * sizeof(char)), ref DIGIT_TABLE[c], 2);
            i += 2;
        }
        if (output2 >= 10)
        {
            ...
            Unsafe.AddByteOffset(ref result, (nint)(index + olength - i) * sizeof(char)) = DIGIT_TABLE[c + 1];
            Unsafe.Add(ref result, index) = DIGIT_TABLE[c];
        }
        else
        {
            Unsafe.Add(ref result, index) = (char)('0' + output2);
        }

        // Print decimal point if needed.
        if (olength > 1)
        {
            Unsafe.Add(ref result, index + 1) = '.';
            index += (int)olength + 1;
        }
        else
        {
        ...
        {
            int32_t c = exp % 10;
            memcpy(ref Unsafe.Add(ref result, index), ref DIGIT_TABLE[2 * (uint)(exp / 10)], 2);
            Unsafe.Add(ref result, index + 2) = (char)('0' + c);
            index += 3;
        }
        else if (exp >= 10)
        {
            memcpy(ref Unsafe.Add(ref result, index), ref DIGIT_TABLE[2 * (uint)exp], 2);
            index += 2;
        }
        else
        {
            Unsafe.Add(ref result, index++) = (char)('0' + exp);
        }
    ...
    public static int d2s_buffered_n(double f, ref char result)
    {
        ...
        {
            return copy_special_str(ref result, ieeeSign, ieeeExponent != 0, ieeeMantissa != 0);
        }
        ...
        return to_chars(v, ieeeSign, ref result);
    }

    public static void d2s_buffered(double f, ref char result)
    {
        int index = d2s_buffered_n(f, ref result);
